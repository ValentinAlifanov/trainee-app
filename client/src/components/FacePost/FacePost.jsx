...
export default function FacePost ({ popularPost, setPopularPost }) {
    const { userAuth } = useContext(UserAuthContext);

    const [user, setUser] = useState({})

    useEffect(() => {
        axios.get(`http://localhost:5000/api/article/postPopular`)
            .then((response) => {
        ...
        <div className='FacePost-box'>
            ...
            <article className='FacePost-read-box'>
                ...
                <div className='FacePost-topic-box'>
                    {userAuth ? (
                        <Link className="FacePost-Link" to={`/readPost/${popularPost._id}`}>
